 ...
 */
export function AVLTree(this: any, options) {
 ...
 */
function _AVLTree(this: any, options: Record<string, unknown> = {}) {
  this.left = null;
  this.right = null;
 ...
 */
Object.assign(_AVLTree, BinarySearchTree);
  ...
  while (true) {
    ...
    if (currentNode.compareKeys(currentNode.key, key) === 0) {
      if (currentNode.unique) {
        const err: any = new Error("Can't insert key " + key + ', it violates the unique constraint');
        err.key = key;
